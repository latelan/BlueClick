...
BOOL CDlgDownloadList::OnInitDialog() 
{
	...
	m_listDownload.SetColumnType(4, BUFFREE_COLUMN_PROGRESS);//设置第四列显示进度条
	...
	m_listDownload.InsertColumn(0, "文件标识", LVCFMT_LEFT, 0, 0);
	m_listDownload.InsertColumn(1, "类型", LVCFMT_LEFT, 100, 1);//第二列
	m_listDownload.InsertColumn(2, "文件名", LVCFMT_LEFT, 250, 2);
	m_listDownload.InsertColumn(3, "大小", LVCFMT_LEFT, 70, 3);
	m_listDownload.InsertColumn(4, "进度", LVCFMT_LEFT, listRect.Width()-438, 4);//通过计算，使得最后一列刚好填满列表宽度
	m_listDownload.InsertColumn(5, "状态", LVCFMT_LEFT, 0, 5);
